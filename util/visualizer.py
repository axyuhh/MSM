...
def save_images(webpage, images_list, image_path, title=None, width=256):
    ...
    for label, img in images_list:
        if os.path.exists(os.path.join(image_dir, label)):
            save_path = os.path.join(image_dir, label, image_path)  # save_path = os.path.join(image_dir, image_name)
...
def save_images(webpage, images_list, image_path, title=None, width=256):
    ...
    for label, img in images_list:
        if os.path.exists(image_dir):
            save_path = os.path.join(image_dir, image_path + '{}.png'.format(ddd))  # save_path = os.path.join(image_dir, image_name)
            util.save_image(img, save_path)
        else:
...
class Visualizer():
    ...
    def display_current_results(self, visuals, epoch, ncols=2, save_result=False, image_format='jpg'):
        ...
        if self.use_html and save_result:  # save images to a html file
            for label, image_numpy in visuals.items():
                img_path = os.path.join(self.img_dir, 'epoch%.6d_%s.%s' % (epoch, label, image_format))
            ...
            for n in range(epoch, 0, -1):
                ...
                for label, image_numpy in visuals.items():
                    img_path = 'epoch%.6d_%s.%s' % (n, label, image_format)
                    ims.append(img_path)
                    txts.append(label)
